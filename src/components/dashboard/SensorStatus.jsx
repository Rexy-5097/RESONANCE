...
const StatusCard = ({ icon: Icon, label, status, type }) => {
    ...
    return (
        <CyberCard className="flex items-center justify-between group">
            <div className="flex items-center gap-4 z-10">
                <div className={clsx(
                    "p-3 rounded-full border transition-colors duration-300",
                    isOk ? "bg-emerald-500/10 border-emerald-500/50 text-emerald-400" : "bg-red-500/10 border-red-500/50 text-red-400"
                )}>
                    <Icon size={20} />
                </div>
                <div>
                    <h4 className="text-slate-400 text-xs font-bold uppercase tracking-wider glow-text">{label}</h4>
            ...
            {/* Decorative BG Icon */}
            <Icon className="absolute -right-4 -bottom-4 text-slate-800/10 transform rotate-12 group-hover:scale-110 transition-transform duration-500" size={80} />
        </CyberCard>
    );
};
